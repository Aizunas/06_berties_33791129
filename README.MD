...
valid email, username length 5-20, password ≥8 characters, first/last names required

### Login: 
username and password not empty

### Add book: 
book name required (1-100 chars), price must be a valid number
...
### Audit Logging
Every login attempt (whether successful or failed) is recorded in the audit_log table with username, success/failure, IP address and timestamp. The full history can be viewed at /users/audit (login required).
